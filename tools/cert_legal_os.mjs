#!/usr/bin/env node
/**
 * tools/cert_legal_os.mjs — Chitti Legal OS (CEOS v1.0) live certification.
 * Self-contained: starts its own static server, then a real Chromium pass.
 * Proves the Vaani #lang-select language dropdown FIRES, the 5 frontend gates,
 * four-user journeys, the deterministic engine, axe-core 0 serious/critical,
 * responsive @375/768/1280. Run: node tools/cert_legal_os.mjs
 */
...
const URL = `http://127.0.0.1:${PORT}/chitti_legal_os.html`;
const URLJ = URL + '?dp_skip=1';   // returning-user: suppress first-visit disability modal
...
const html = readFileSync(resolve(ROOT, 'chitti_legal_os.html'), 'utf8');
...
for (const v of [{ name: '375', w: 375, h: 812, dsf: 2 }, { name: '768', w: 768, h: 1024, dsf: 2 }, { name: '1280', w: 1280, h: 900, dsf: 1 }]) {
  await safe('screenshot_' + v.name, async () => {
    ...
    const out = resolve(SHOT_DIR, `chitti_legal_os_${v.name}.png`);
...
check('BO1 role=tab tabs (8)', await page.locator('[role="tab"]').count() === 8);
check('BO1 aria-live result hosts ≥8', await page.locator('[aria-live="polite"]').count() >= 8);

// ── 5. Four-user TAP-ONLY journeys + engine functional ──
await safe('ENGINE rights coach renders rights', async () => {
  await page.click('#tab-rights');
  await page.click('button[onclick="lglRights()"]');
  await page.waitForTimeout(500);
  const txt = await page.locator('#r-rights').innerText();
  check('ENGINE rights lists rights + law', /right|police|BNSS|Article/i.test(txt), txt.split('\n')[0]);
  const word = await page.locator('#r-rights .res-status').innerText().catch(() => '');
  check('BO2 result has symbol+WORD status', /Good|Check this|Note/.test(word), word.trim());
  check('BO4 read-aloud button on result', await page.locator('#r-rights .speak-btn').count() >= 1);
});
await safe('ENGINE deadline (limitation) renders a date', async () => {
  await page.click('#tab-dead');
  await page.selectOption('#lm-matter', 'money_recovery');
  await page.fill('#lm-date', '2024-01-01');
  await page.click('button[onclick="lglLimit()"]');
  await page.waitForTimeout(500);
  const txt = await page.locator('#r-limit').innerText();
  check('ENGINE limitation shows a deadline date', /20\d\d-\d\d-\d\d/.test(txt) && /year|deadline/i.test(txt), txt.split('\n')[0]);
});
await safe('ENGINE free legal aid (the moat) renders eligibility', async () => {
  await page.click('#tab-help');
  await page.click('#aid-woman');
  await page.click('button[onclick="lglAid()"]');
  await page.waitForTimeout(500);
  const txt = await page.locator('#r-aid').innerText();
  check('ENGINE legal aid shows eligibility + 15100', /15100/.test(txt) && /eligible|legal aid/i.test(txt), txt.split('\n')[0]);
});
await safe('ENGINE scam shield flags a scam + 1930', async () => {
  await page.click('#tab-scam');
  await page.click('#sc-threatensArrest');
  await page.click('#sc-asksOtpOrPin');
  await page.click('button[onclick="lglScam()"]');
  await page.waitForTimeout(500);
  const txt = await page.locator('#r-scam').innerText();
  check('ENGINE scam flags high-risk + tells 1930', /1930/.test(txt) && /high-risk|scam/i.test(txt), txt.split('\n')[0]);
});
...
console.log(`\nChitti Legal OS cert — ${pass}/${R.length} GREEN, ${fail} failed`);
