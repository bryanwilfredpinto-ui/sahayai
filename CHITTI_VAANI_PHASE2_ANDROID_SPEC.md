...
| 5b | **24/7 emergency cascade — family only, never cops** | Web Speech continuous listening + Web Audio alarm + `/api/vaani/emergency/trigger` relay to paired Chittis | Vosk on-device keyword spotter foreground service + `STREAM_ALARM` bypass + FCM push for inbound relay. Cascade: confirm-master 10s → ring alarm bypass-silent 10s → outbound call to spouse/family from trusted circle. **Refuses to auto-dial 112/100/102/108/1098/1930/139.** |
