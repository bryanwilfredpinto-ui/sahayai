...
3. Fill in:
   ...
   - **Redirect URL**: `https://chitti-shares-api.onrender.com/api/market/auth-callback` *(must match exactly — copy-paste this)*
...
curl -X POST https://chitti-shares-api.onrender.com/auth/verify-otp \
  -H "Content-Type: application/json" \
...
curl https://chitti-shares-api.onrender.com/api/market/auth-url \
...
```bash
curl https://chitti-shares-api.onrender.com/api/market/auth-status \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN_HERE"
```
...
```bash
curl https://chitti-shares-api.onrender.com/api/market/indices \
...
```bash
curl https://chitti-shares-api.onrender.com/api/market/view \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
```bash
curl -i https://chitti-shares-api.onrender.com/api/market/indices
# expect: HTTP/2 401
```
...
| API docs | https://chitti-shares-api.onrender.com/docs |
