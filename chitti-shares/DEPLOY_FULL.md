...
**Backend API**: https://chitti-shares-api.onrender.com
...
KITE_API_SECRET=
KITE_REDIRECT_URL=https://chitti-shares-api.onrender.com/api/market/auth-callback
DATA_SOURCE=yahoo               # 'yahoo' (free) or 'kite' (paid). Switch later by changing this.
DEEPSEEK_API_KEY=               # From platform.deepseek.com
FRONTEND_URL=https://shares.sahayai.in
BACKEND_URL=https://chitti-shares-api.onrender.com
```
...
6. **Environment**: add `VITE_API_URL=https://chitti-shares-api.onrender.com` (no trailing slash).
7. Deploy.
...
|---|---|---|---|
| chitti-alerts | `*/5 3-10 * * 1-5` | every 5 min, Mon–Fri 8:30–16:00 IST (covers full market) | `curl -X POST 'https://chitti-shares-api.onrender.com/api/cron/alerts?secret=$CRON_SECRET'` |
| chitti-track-calls | `*/5 3-10 * * 1-5` | same as above | `curl -X POST 'https://chitti-shares-api.onrender.com/api/cron/track-calls?secret=$CRON_SECRET'` |
| chitti-kite-reauth | `25 0 * * *` | 5:55 AM IST daily | `curl -X POST 'https://chitti-shares-api.onrender.com/api/cron/kite-reauth?secret=$CRON_SECRET'` |
...
```bash
# Backend health
curl https://chitti-shares-api.onrender.com/health
# → {"ok":true}

# OTP flow (replace MOBILE)
curl -X POST https://chitti-shares-api.onrender.com/auth/send-otp \
     -H "Content-Type: application/json" \
     -d '{"mobile":"9876543210"}'
# → {"ok":true}  (real SMS arrives if FAST2SMS_API_KEY is set)

# Cron secret check
curl -X POST 'https://chitti-shares-api.onrender.com/api/cron/alerts?secret=wrong'
# → 401
...
```bash
BASE=https://chitti-shares-api.onrender.com
MOBILE=9876543210
...
This is by design. Zerodha invalidates all access tokens daily ~06:00 IST. The Telegram cron at 5:55 IST sends a reminder. To re-auth: visit `https://chitti-shares-api.onrender.com/api/market/auth-url` while logged in as `ADMIN_MOBILE`.
