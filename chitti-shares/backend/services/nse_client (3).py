    ...
    if _client is None or (now - _cookies_set_at) > _COOKIE_TTL:
        if _client is not None:
            try:
                ...
                pass
        c = _new_client()
        # Visit several pages in sequence to look more like a real browser session.
        # NSE checks for plausible referer + cookie chain before serving JSON.
        warmup_urls = [
            "/",
            "/market-data/live-equity-market",
            "/get-quotes/equity?symbol=RELIANCE",
            "/api/marketStatus",  # this lighter endpoint also primes cookies
        ]
        for u in warmup_urls:
            try:
                r = c.get(u, timeout=10.0)
                log.info("[nse] warmup %s -> %s (cookies=%d)", u, r.status_code, len(c.cookies))
            except Exception as e:  # noqa: BLE001
                log.warning("[nse] warmup %s failed: %s", u, e)
            time.sleep(0.5)  # small gap, more human
        _client = c
        _cookies_set_at = now
    return _client
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                try:
                    ...
                    log.warning("[nse] %s -> 200 but non-JSON. Body preview: %s", path, body_preview)
                    raise RuntimeError(f"NSE returned non-JSON for {path}: {body_preview[:120]}")
                return j
...
def get_index_quote(canonical_symbols: list[str]) -> dict:
    ...
    Strategy:
      1. Try NSE direct (works from clean cloud IPs)
      2. Fall back to database (filled by user's laptop pusher script)
    """
    ...
    # Step 1: Try NSE direct (with cache)
    now = time.time()
    ...
    if cached and (now - cached[0]) < 30:
        ...
        try:
            ...
            log.warning("[nse] direct fetch failed, will try DB fallback: %s", e)
            all_idx = {}
    ...
    # Step 2: For anything still missing, fall back to database (laptop pusher)
    missing = [s for s in wanted if s not in out or not out[s].get("last_price")]
    if missing:
        try:
            ...
            db = SessionLocal()
            try:
                rows = db.query(IndexQuote).filter(IndexQuote.canonical.in_(missing)).all()
                for r in rows:
                    if r.last_price:
                        out[r.canonical] = {
                            ...
                            "stale_age_sec": (
                                (datetime.now(timezone.utc).replace(tzinfo=None)
                                 - r.updated_at.replace(tzinfo=None)).total_seconds()
                                if r.updated_at else None
                            ),
                        }
                        log.info("[nse] DB fallback served %s (age=%ss)",
                                 r.canonical, out[r.canonical].get("stale_age_sec"))
            finally:
                db.close()
        except Exception as e:  # noqa: BLE001
            log.warning("[nse] DB fallback failed: %s", e)
...
def _fetch_all_indices() -> dict[str, dict]:
    """
    Hit /api/allIndices first. If that returns non-JSON (NSE bot challenge),
    fall back to per-index calls which are usually more lenient.
    Returns dict keyed by NSE's "index" name string.
    """
    ...
    try:
        ...
        log.warning("[nse] allIndices failed, falling back to per-index calls: %s", e)

    # Fallback: hit /api/equity-stockIndices?index=X for each major index
    # This endpoint is sometimes served when /api/allIndices is rate-limited
    for nse_name, url_name in [
        ("NIFTY 50", "NIFTY%2050"),
        ("NIFTY BANK", "NIFTY%20BANK"),
    ]:
        try:
            j = _get_json(f"/api/equity-stockIndices?index={url_name}")
            # This endpoint returns {"name": "...", "metadata": {...}, "data": [...]}
            # The metadata block has the index-level summary we want
            ...
            if meta:
                by_name[nse_name] = {
                    ...
                    "open": meta.get("open"),
                    "dayHigh": meta.get("high"),
                    "dayLow": meta.get("low"),
                    "change": meta.get("change"),
                    "percentChange": meta.get("percChange"),
                }
                log.info("[nse] per-index fallback worked for %s", nse_name)
        except Exception as e:  # noqa: BLE001
            log.warning("[nse] per-index fallback failed for %s: %s", nse_name, e)
    return by_name
