...
/tmp/chitti_shares.db via stdlib sqlite3 and lost every row on Railway restart
(wal_insert_begin failed; sync to Turso never succeeded).

Replacement: direct HTTPS via lib.turso_http (PEP-249 shim talking to Turso
/v2/pipeline). Plugged into SQLAlchemy via create_engine(..., creator=...).
No local file, no background sync, no /tmp anywhere.
