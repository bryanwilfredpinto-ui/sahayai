...
# NOTE: raw OHLC candles for the standalone Chitti Technical page are served by the
# existing public endpoint GET /api/candles/{symbol}?timeframe=...&days_back=... in main.py
# (Angel-backed, supports Daily/Weekly/Monthly/4H/1H). No separate route needed here — a
# /{symbol:path}/candles route would be shadowed by the catch-all @app.get("/api/technical/{symbol:path}").
