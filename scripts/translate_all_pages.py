...
CORPUS = REPO / "scripts" / "all_pages_corpus.json"
TRANSLATIONS = REPO / "scripts" / "all_pages_translations.json"
SEED = REPO / "scripts" / "vaani_translations.json"  # reuse vaani's existing translations
...
def load_existing() -> dict:
    # Start from any prior all_pages_translations.json (resumable), then
    # overlay vaani_translations.json so vaani's already-translated strings
    # don't need to be re-fetched.
    data = {}
        ...
        try:
            ...
            for k, v in seed.items():
                if k not in data:
                    ...
                    # Merge per-lang values without overwriting newer ones.
                    for lc, val in v.items():
                        data[k].setdefault(lc, val)
        except json.JSONDecodeError:
            pass
    return data
