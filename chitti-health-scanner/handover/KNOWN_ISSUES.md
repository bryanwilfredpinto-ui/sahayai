...
|---|---|---|---|---|---|
| **KI-01** | **16 MB `chitti_lang.js`** loads whole → full-asset `load` > 30 s on real 3G | 🟠 High **tech-debt** (platform-wide, not page-specific) | First visit on a slow (≤ ~400 kbps) connection, before assets finish | Page is **interactive at DOMContentLoaded (~1 s)** and usable before full load; on Wi-Fi/4G it's ~1 s total | Split the i18n dictionary per-language / lazy-load the active language only. Biggest single perf + scalability win. Affects every Chitti page. |
...
- **Performance bottlenecks:** one — KI-01 (16 MB i18n on 3G). Everything else is within target (load ~1 s, switch 103 ms, heap 23 MB).
