"""Ingest — build the Chitti Legal vector DB from official legal texts.
...
is stored with {doc, source, url, section, page} so answers can cite the exact Act +
section/article + page and link the official source. PDF files named `<id>.pdf` are
matched back to their OFFICIAL_SOURCES entry so the citation shows the proper Act name
+ official URL (not the bare filename). Never fabricates content — if a download is
blocked it logs the failure and skips that source.
"""
...
log = logging.getLogger("legal_rag.ingest")

# id -> {doc, source, url} so PDF chunks cite the real Act, not the filename stem.
_META_BY_ID = {s["id"]: s for s in OFFICIAL_SOURCES}
...
def _download_one(src: dict) -> Path | None:
    ...
    log.warning("  ✗ %s: all sources failed — corpus will rely on what is already in rag/corpus/ "
                "until this is fetched manually. Source of record: %s", src["id"], src["url"])
    return None


def download_all() -> int:
    log.info("Downloading official legal sources → %s", config.CORPUS_DIR)
    got = 0
...
def build(reset: bool = False, force_fallback: bool = False) -> dict:
    ...
    for p in files:
        meta = _META_BY_ID.get(p.stem, {})
        chunks = chunk_path(p, doc=meta.get("doc", ""), source=meta.get("source", ""),
                            url=meta.get("url", ""))
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Build Chitti Legal RAG vector DB")
    ...
    if args.download:
        download_all()
    res = build(reset=True, force_fallback=args.fallback)
    print(res)
