...
log = logging.getLogger("legal_rag.retriever")


def _ref(meta: dict) -> str:
    sec = (meta.get("section") or "").strip()
    page = (meta.get("page") or "").strip()
    doc = meta.get("doc") or meta.get("source") or "official legal text"
    bits = []
    if sec and sec != "—":
...
def rag_status() -> dict:
    """Cheap health/status for /api/legal/rag/health — does NOT embed anything."""
    embedder = get_embedder()
    store = get_store()
    return {
        "ready": store.count() > 0,
        ...
        "note": ("Semantic RAG ready." if embedder.semantic and store.count()
                      ...
                      "ChromaDB + sentence-transformers) or the corpus is empty (run "
                      "python -m rag.ingest --download)."),
    }
