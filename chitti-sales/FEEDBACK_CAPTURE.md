...
**Where it posts.** `POST {API}/api/feedback/collect` — the existing Chitti Vaani feedback endpoint (`https://chitti-vaani-api-production.up.railway.app` by default; override with `window.CHITTI_FEEDBACK_API` before the script loads).
