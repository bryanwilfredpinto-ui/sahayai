...
- **No object storage** — no S3, no R2, no Railway disk write.
- **No analytics, no telemetry, no logging service.** The Railway-default stdout log is the only observability surface.
...
| Asset storage | **Cloudflare R2** (recommended — zero egress fees) > AWS S3 > Railway disk. | New `backend/services/storage.py` (does not exist yet) | TBD |
