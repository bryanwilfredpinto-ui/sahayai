...
log = logging.getLogger("ca_rag.embedder")
...
class _HashingEmbedder:
    ...
    def _vec(self, text: str) -> list[float]:
        ...
        for tok, c in tf.items():
            ...
            v[h] += 1.0 + math.log(c)
            # add char-4gram features ONLY for longer tokens (cuts spurious overlap of
            # short words like "good"/"goa" while still catching section/registration).
            if len(tok) >= 7:
