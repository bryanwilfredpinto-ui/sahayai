"""
chitti-2wheeler / backend / services / deepseek_client.py
---------------------------------------------------------
Single DeepSeek call helper. Loads the SKILL.md persona + the
MECHANIC_KNOWLEDGE.md corpus as the system prompt prefix. Enforces the
server-side disclaimer on every reply so the client can never strip it
(per [SAHAYAI_MASTER §6 quality rule — server-enforced disclaimers]).

If DEEPSEEK_API_KEY is unset, returns an honest fallback reply rather
than inventing answers. Per the [Honest stubs over fake demos] rule.
"""
...
# Resolve skills/ relative to this file: backend/ → ../skills/
...
SYSTEM_PROMPT = textwrap.dedent(
    """
    You are **Chitti 2-Wheeler** — Bharat's voice-first agent for
    motorcycle and scooter owners. Speak simple Hinglish by default.
    You are *a guardian, a commando, a coach* — not a polite assistant.

    Hard rules:
    - You are NOT a mechanic. Every answer is guidance.
    - Quote price BANDS (₹350–500), never a single final number.
    - Emergency = family cascade. NEVER tell the user to dial 100 / 108 / 112.
...
def _raw_2w_deepseek(profile_blurb: str, safe_text: str) -> tuple[str, dict, int]:
    payload = {
...
def ask(question: str, profile: dict | None = None, lang_hint: str = "hinglish") -> dict[str, Any]:
    """
    Send a single-turn question to DeepSeek. Returns
    `{"answer": str, "error": Optional[str], "cost": {...}}`.
    ...
    `profile` is the user's bike profile (brand, model, year, odometer);
    we pass it as additional context so Chitti can personalise.
    """
    if not settings.DEEPSEEK_API_KEY:
        return {
            "answer": (
                "DeepSeek abhi configured nahi hai backend pe. Maintenance schedule + "
                "breakdown coach + RSA numbers without DeepSeek bhi chalte hain — un tabs "
                "pe jao. Founder ko bata diya jayega ke key add karen."
                + DISCLAIMER
            ),
    ...
    if profile and isinstance(profile, dict):
        ...
        for k in ("brand", "model", "year", "fuel", "odo", "reg"):
            ...
            if v:
                bits.append(f"{k}={v}")
        if bits:
            profile_blurb = "User's bike: " + ", ".join(bits) + ".\n\n"
    ...
    def _call(safe_text: str) -> str:
        ans, usage, status = _raw_2w_deepseek(profile_blurb, safe_text)
    ...
    try:
        ans, usage, status = _raw_2w_deepseek(profile_blurb, question.strip())
        usage_capture.update(usage)
        if status != 200:
            return {
                "answer": "Backend hiccup hua. Phir try karo." + DISCLAIMER,
