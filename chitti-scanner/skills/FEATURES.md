...
| S2 | **Verify frontend `API_BASE`** points to the right Railway slug after deploy | **P0** | `chitti_scanner.html` defaults to `https://chitti-scanner-api.onrender.com`; if Railway assigns a different slug we need to either rename the service or update the constant. | Live-page click after S1. |
