...
| **Scope** | **Does:** Jan Aushadhi lookup, branded ↔ generic comparison, family wallet, prescription / strip scan (DeepSeek vision), expiry reminders, price alerts (P1), health-file records (Skill 9), non-diagnostic visual health-scan — skin/mole/wound/eye/dental (Skill 10). **Does NOT:** diagnose or name any disease, recommend dose changes, sell or fulfil medicines, substitute for a pharmacist or doctor. |
...
> **Chitti Health Scanner = MedUPI Skill 10, NOT a standalone Chitti (merged 2026-06-24).** The visual health-scan capability (skin / mole / wound / eye / dental assessment via DeepSeek vision, Guardian Memory longitudinal tracking, doctor-referral + emergency-escalation SOPs — see [chitti-health-scanner/](chitti-health-scanner/)) is **Skill 10 inside Chitti MedUPI** — reached via Vaani → MedUPI → Health Scanner, never as its own product. It is **strictly non-diagnostic**: it NEVER names a disease, surfaces an honest `coming_soon` where AI vision is unbuilt (501), and always routes anything clinical to a doctor (doctor-referral / emergency-escalation SOPs are merge-blockers). The same HIGH-risk Swarm gate, camera-intelligence contract (§2b), and per-response widget that govern MedUPI apply unchanged. `chitti-health-scanner/` (COSDF doc set) + `chitti_health_scanner.html` stay as **reference + dev/debug surface only**; this rule does not promote Health Scanner to a 16th Chitti. Same standalone-out-of-scope rule as Health File (Skill 9) above.
...
| Field | Value |
|---|---|
| **Objective** | Digital mechanic companion (Chitti Bike Doctor): *"Do I need a mechanic, can I fix it myself, and is this quote fair?"* — plus the full 15-feature ownership OS (vault · reminders · inspection · insurance · PUC · service/oil · tyre · battery · fuel/EV · scam · DIY triage · sell · savings), for 2-wheeler owners. Full CEOS spec: [chitti-mechanic-2w/ceos/](chitti-mechanic-2w/ceos/) (45 docs, CEOS v1.0 ground-up rebuild 2026-06-13). Deterministic engine [`chitti_mechanic_2w_engine.js`](chitti_mechanic_2w_engine.js). **Legacy `chitti-2wheeler/` retired 2026-06-24 — `chitti-mechanic-2w/` is canonical.** |
...
| **Quality standard** | **Deterministic engine is the product** — engine **92/92** gold assertions + live Playwright cert **38/38** (axe 0 serious/critical; 26-lang Vaani switch; 5 device screenshots): Safety =100% (critical safety errors=0) · DIY-safety unsafe-recs=0 · Cost-band ≥85% · **never claim certainty** (confidence bands) · six-field answer (Why/Severity/Can-ride/DIY-tier/Cost/Alternatives). DeepSeek/vision/live-API are honest COMING SOON with deterministic local equivalents (doc-upload, .ics reminders, IDV premium estimate, deterministic triage). Per-response widget on every card; 7 LIVE deterministic backend endpoints. Frontend on [chitti_mechanic_2w.html](chitti_mechanic_2w.html). **Eval numbers measured, never claimed before measured.** |
...
| **Evolution owner** | [chitti-mechanic-2w/ceos/skills/FEATURES.md](chitti-mechanic-2w/ceos/skills/FEATURES.md) + the CEOS doc set ([ROLE.md](chitti-mechanic-2w/ceos/ROLE.md), [BUILD_ORDER.md](chitti-mechanic-2w/ceos/BUILD_ORDER.md), [swarm/](chitti-mechanic-2w/ceos/swarm/)). HIGH-risk (safety) changes require Sire's approval. |
