...
> repair, the driver tells Chitti what the mechanic *actually* fixed. That ground truth
> is the highest-quality signal in the whole system — it turns Chitti from a guesser
> into a learner.

## The flow
1. Chitti gives a diagnosis with a confidence band (e.g. *"Likely alternator — High"*).
2. The driver goes to a mechanic / fixes it.
3. Later, Chitti asks **one** simple, tappable question:
   > *"Mechanic ne kya theek kiya?"*
   > 🔋 Battery · ⚡ Alternator · 🔌 Starter · 🕯️ Plugs/Coils · ⛽ Fuel pump ·
   > 🛑 Brakes · ❄️ AC · 🌡️ Coolant/Overheat · 🛞 Tyre · 🔧 Other
4. The driver taps (or speaks) the real fix — and optionally the real cost.
...
| DTC interpreted vs actual fault | validates / corrects the P-code → cause mapping |
...
a pattern (e.g. "Swift + battery-light-while-driving + dim-at-idle ⇒ alternator") is
validated and pushed to [../skills/](../skills/). HIGH-risk safety patterns get human
review first. Every Chitti Car Doctor instance then benefits from one driver's real
outcome.

## Privacy (non-negotiable)
- Fully **anonymised** — no driver identity, no plate, no location in the aggregate.
- Stored on-device first; only the anonymised predicted/actual/cost tuple leaves.
- `"Chitti forget"` tombstones the driver's contribution.
- Mechanic / service-centre name (if ever given) is **never** stored against a quality
  verdict — no defamation, ever ([../guardrails/scam-shield-rules.md](../guardrails/scam-shield-rules.md)).

## Accessibility
The follow-up is **one tap on an icon menu** (mute/illiterate-safe), spoken aloud
(blind-safe), with symbols + words + ISL (deaf-safe). Never a text form. The driver can
