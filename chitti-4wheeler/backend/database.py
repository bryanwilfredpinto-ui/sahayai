"""
chitti-4wheeler / backend / database.py
---------------------------------------
SQLAlchemy engine + session + ensure_schema(). Mirrors the
chitti-news + chitti-2wheeler Turso embedded-replica pattern locked
2026-05-12 in [[project_turso_embedded_replica_pattern]].
...
def _resolve_url(raw: str) -> str:
    ...
    if raw.startswith("libsql://"):
        local = os.environ.get("LIBSQL_REPLICA_PATH", "/tmp/chitti_4wheeler.db")
        _REPLICA_PATH = local
