...
Car Doctor.** This file is the **index + contract** for the four-user accessibility floor. The
...
> A car breaks down for blind, deaf, mute and illiterate drivers (and their families) too — and they
> have **no app** to help them. Chitti does. Accessibility is the **floor under everything**, not an
> asterisk ([SAHAYAI_MASTER.md §7](../SAHAYAI_MASTER.md)). A safety verdict a deaf driver can't perceive,
> or a diagnosis a blind driver can't hear, is a **defect** — not a limitation.

## The per-user docs (index — detail in `accessibility/`)

| User | Hero path | Detail doc |
|---|---|---|
| 👁️ Blind | "Describe my dashboard" → sound-first, voice-out diagnosis | [accessibility/blind_user.md](accessibility/blind_user.md) |
| 🦻 Deaf | visual-first; text + symbols + ISL; 🔴 unmissable | [accessibility/deaf_user.md](accessibility/deaf_user.md) |
| 🤫 Mute | whole diagnosis by photo + tap; tap-confirm SOS/RSA | [accessibility/mute_user.md](accessibility/mute_user.md) |
...
|---|---|---|---|
| 👁️ Blind | voice + touch | voice + haptic | "Describe my dashboard" narrates the warning lights + OBD2 code; spoken diagnosis; **safety call spoken FIRST**; "say HAAN when done" stepping; auto-announce on open |
| 🦻 Deaf | touch + camera | visual + text | captions per line; verdict = symbol **+ word** (never colour-only); ISL panel per box; 🔴 (overheat / brake) = symbol + word + **screen flash**; photo→code→meaning flow fully visual |
| 🤫 Mute | touch + camera + presets | visual + voice | photo of the cluster + tap symptom picker; Golden-Rule confirm accepts a **tap**; family-cascade SOS and RSA dial by big tap button |
| 📖 Illiterate | voice + camera + thumbs | voice + icons | every label spoken (incl. the P-code's *meaning*); picture symptom menu; "say HAAN" + big tap; works on 2G |
...
|---|---|---|
| **Voice-First** | no visual dependency; confirm sounds; haptic | spoken cause + safety + DIY + cost band; even the OBD2 P-code's meaning spoken plainly; "say HAAN" stepping; `chitti_a11y.js` read-page |
| **Visual-First** | captions, flashing-red border, icons, ISL/caption guides, colour **+ word** | per-line captions; 🔴/🟠/🟢 symbol+word; `chitti_isl.js` panel per `data-chitti-response` box; **screen flash** on a 🔴 verdict (overheat / brake failure) a deaf driver cannot miss |
| **Icon-First** | 🎤 📷 👍 👎 🔊 🚨 — no text required | picture symptom menus (🔋 dead dashboard, ⛽ no fuel, 💨 smoke, 🌡️ overheat, 🔊 noise, 🛑 brake); tappable narrowing questions; SOS as a single button |
...
sentence ("aapka portfolio dekho" is banned). Technical terms stay in English (OBD2, DTC/P0420, ABS,
SRS, RPM, DPF, EGR, AC, EV, brand/model names) and are translated nowhere — they are fingerspelled in
ISL and pronounced as English letters in TTS ([CTO.md §5-6](../chitti-cto/CTO.md)).
...
| **Manual — blind** | TalkBack pass: onboard a car by voice → "engine se awaaz" → hear ranked causes + confidence → hear safety call → hear DIY tier + cost; **zero** sighted assistance |
| **Manual — deaf** | muted device: full diagnosis by text + symbols + ISL; a 🔴 overheat verdict unmistakable (symbol + word + flash); ISL panel on every box |
| **Manual — mute** | mic disabled: onboard → photo dashboard → tap symptoms → tap-confirm an RSA dial → tap-fire family SOS |
| **Manual — illiterate** | 2G throttle, regional language, no-reading: complete the breakdown flow ("gaadi start nahi ho rahi") by ear + pictures, incl. tap/voice family SOS |
...
- **Voice in + voice out + symbols + plain language** — never colour-only, never audio-only, never
  reading-required. A dashboard light / OBD2 code shown but not narrated is a defect for a blind driver.
...
- **Golden Rule (§2g):** every side-effecting action (SOS / RSA dial / alarm) gates on
  `chittiConfirmAndDo()` and accepts a **tap or a spoken HAAN** — never voice-only, never auto-Yes. A
  stranded mute driver must be able to call family with a tap.
...
- **EV HV safety carries through every modality:** a 🔴 "DO NOT TOUCH orange HV cables" call is spoken
  first (blind), shown as symbol + word + flash (deaf), and never reduced to colour.
